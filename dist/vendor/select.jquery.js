...
;(function($){
	$.fn.multiselect=function(options){
		var defaults = {
			...
			selectedIcon: '<i class="glyphicon  glyphicon-menu-up arrow"></i>',
		...
		this.each(function(){
			...
			_t.find('option').each(function(i){
				...
				// console.log(_this.html().substr(1,options.setMaxOptionNum))
				_thisHtmlValue = _this.html();
				...
				if(options.multiple){
					...
					newOptions += "<li class='select1' data-value='"+_thisValue+"' "+ selectedOption +" >"+ _thisHtmlValue + selected +"</li>";								// 创建新的options
			...
			newSelectTitle = '<lable class="newSelectTitle"><span>'+a+'</span><i class="glyphicon  glyphicon-menu-down arrow"></i></lable>';			// 创建新的selectTitle
